...
int sc_fram_clear_tlm_seq_num(void)
{
	...
	ret = sc_fram_write(SC_FRAM_MEM0, FRAM_TLM_SEQ_NUM_ADDR, sizeof(tlm_seq_num), (uint8_t *)&tlm_seq_num);
	if (ret < 0) {
...
int sc_fram_update_tlm_seq_num(void)
{
	...
	ret = sc_fram_read(SC_FRAM_MEM0, FRAM_TLM_SEQ_NUM_ADDR, sizeof(tlm_seq_num), (uint8_t *)&tlm_seq_num);
	if (ret < 0) {
...
{
	int ret = sc_fram_read(SC_FRAM_MEM0, FRAM_TLM_SEQ_NUM_ADDR, sizeof(*tlm_seq_num), (uint8_t *)tlm_seq_num);
	if (ret < 0) {
